...
function UploadPage() {
  ...
  const handleTranslate = async () => {
    ...
    formData.append("file", file);
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/translate-pdf`, {
        method: 'POST',
        headers: {
...
if (response.ok && data.translated_text) {
  ...
  try {
    ...
    await addDoc(collection(db, 'translations'), {
      userId: user.uid,
      sourceText: fileName || "PDF File",
      translatedText: data.translated_text.substring(0, 500),
      method: 'File',
      createdAt: new Date(),
    });
    ...
    console.log("Firestore save failed:", firebaseError);
  }

} else {
  setTranslatedText(data.error || 'Translation failed');
} 
    }
    /*catch (err) {
  ...
  // DO NOT overwrite successful translation
  if (!translatedText) {
    setTranslatedText('Error: Could not translate.');
  }
  }
